...
#define RTKImageVersionInvalid  0xffff

/**
 * Constants that represents upgrade method.
 */
typedef enum : uint8_t {
    RTKOTAUpgradeMethod_default =   0x00,       ///< Using normal upgrade method
    RTKOTAUpgradeMethod_OTATempSection,         ///< Use OTA temp section for upgrade
} RTKOTAUpgradeMethod;
...
extern const uint8_t RTKDFUBatteryLevelInvalid;
 ...
 */
- (void)getOTAInformationWithCompletionHandler:(nullable void(^)(BOOL success, NSError *__nullable error, RTKOTADeviceInfo *__nullable info))handler;
 ...
 */
- (void)getProtocolTypeWithCompletionHandler:(nullable void(^)(BOOL success, NSError *__nullable error, NSUInteger protocol))handler;
 ...
 *
 * @param handler The completion handler to call when the request is complete.
 *      @param battery When request succeed, battery is the charge level of connected device.
 *      @param secondaryBattery When request succeed and the device has a secondary unit, this value is the charge level of the secondary unit.
 */
 ...
 * @param method The method to used for upgarde.
 * @param encrypt Whether encrypt this payload data when sending.
...
- (void)startUpgradeWithImageHeaderData:(NSData *)headerData
                          upgradeMethod:(RTKOTAUpgradeMethod)method
 ...
 *
 * @param dataSlice The bytes fragment which is about to be send.
 * @param handler The completion handler to call when the request is complete.
 */
